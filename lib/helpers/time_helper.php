  ...
  public static function get_timezone_name_from_session(){
    if(self::is_timezone_saved_in_session()){
      ...
      if(OsAuthHelper::is_customer_logged_in()){
        $customer_timezone_name = OsMetaHelper::get_customer_meta_by_key(LATEPOINT_SELECTED_TIMEZONE_COOKIE, OsAuthHelper::get_logged_in_customer_id());
        if(!empty($customer_timezone_name)){
          $timezone_name = $customer_timezone_name;
        }else{
          OsMetaHelper::save_customer_meta_by_key(LATEPOINT_SELECTED_TIMEZONE_COOKIE, self::get_wp_timezone_name(), OsAuthHelper::get_logged_in_customer_id());
          $timezone_name = self::get_wp_timezone_name();
        }
      }else{
        $timezone_name = self::get_wp_timezone_name();
      }
      self::set_timezone_name_in_cookie($timezone_name);
    }
