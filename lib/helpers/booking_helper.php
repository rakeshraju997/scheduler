...
class OsBookingHelper
{
  ...
  public static function widget_day_info($target_date, $args = [])
  {
    ...
    foreach ($agents_ids as $agent_id) {
      $work_periods = OsBookingHelper::get_work_periods(array(
        'custom_date' => $target_date->format('Y-m-d'),
        'week_day' => $target_date->format('N'),
...
?>
  ...
  public static function generate_services_list($services = false, $preselected_service = false)
  {
    if ($services && is_array($services) && !empty($services)) { ?>
      <div class="os-services os-animated-parent os-items os-as-rows os-selectable-items">
        ...
        ?>
          <div class="os-animated-child os-item os-selectable-item <?php echo ($preselected_service && $service->id == $preselected_service->id) ? 'selected is-preselected' : ''; ?> <?php echo ($is_priced) ? 'os-priced-item' : ''; ?> <?php if ($service->short_description) echo 'with-description'; ?> <?php if ($service->get_extra_durations()) echo 'has-multiple-durations has-child-items'; ?>" data-item-price="<?php echo $service->charge_amount; ?>" data-priced-item-type="service" data-summary-field-name="service" data-summary-value="<?php echo esc_attr($service->name); ?>" data-item-id="<?php echo $service->id; ?>" data-min-capacity="<?php echo $service->capacity_min; ?>" data-max-capacity="<?php echo $service->capacity_max; ?>" data-activate-sub-step="<?php echo $activate_sub_step; ?>" data-id-holder=".latepoint_service_id">
            <div class="os-service-selector os-item-i os-animated-self" data-min-capacity="<?php echo $service->capacity_min; ?>" data-max-capacity="<?php echo $service->capacity_max; ?>" data-service-id="<?php echo $service->id; ?>">
            ...
            <?php if (count($service_durations) > 1) { ?>
              <div class="os-service-durations os-animated-parent os-items os-as-grid os-three-columns os-selectable-items">
                ...
                foreach ($service_durations as $extra_duration) {
                  ...
                  <div class="os-duration-item os-animated-child os-selectable-item os-item with-floating-price <?php echo ($is_priced) ? 'os-priced-item' : ''; ?>" data-item-price="<?php echo $extra_duration['charge_amount']; ?>" data-priced-item-type="service" data-summary-field-name="duration" data-min-capacity="<?php echo $service->capacity_min; ?>" data-max-capacity="<?php echo $service->capacity_max; ?>" data-previous-sub-step="<?php echo $previous_sub_step; ?>" data-activate-sub-step="<?php echo $activate_sub_sub_step; ?>" data-summary-value="<?php echo esc_attr($summary_duration_label); ?>" data-item-id="<?php echo $extra_duration['duration']; ?>" data-id-holder=".latepoint_duration">
  ...
  public static function generate_services_and_categories_list($parent_id = false, $show_selected_categories = false, $show_selected_services = false, $preselected_category = false)
  {
    ...
    data-os-after-call="latepoint_init_quick_booking_form"';
        ...
        public static function is_timeframe_in_work_periods($timeframe_start, $timeframe_end, $work_periods_arr, $service_available = false)
        {
        ...
        public static function get_bookings_times_for_date($date, $agent_id, $location_id = false, $approved_only = true, $exclude_booking_ids = false, $service_id = false)
        {
          $bookings = new OsBookingModel();
                ...
                if (isset($work_periods_arr['agent_' . $agent_id][$day_date->format('Y-m-d')])) {
                  foreach ($work_periods_arr['agent_' . $agent_id][$day_date->format('Y-m-d')] as $work_period) {
                    ...
                    for ($minute = $period_start; $minute <= $period_end; $minute += $service->get_timeblock_interval()) {
                      ...
                      if (!OsBookingHelper::is_timeframe_in_work_periods($booking_start_minute, $booking_end_minute, $work_period, $service_available)) {
                        if ($select) {
          ...
          <?php if (!$is_day_in_past && !$not_in_allowed_period) { ?>
            <div class="os-day-status">
              ...
              if ($total_work_minutes > 0) {
                ...
                $total_day_minutes_count = count(array_filter($day_minutes, function ($minute) use ($duration_minutes, $work_end_minutes) {
                  return (($minute + $duration_minutes) <= $work_end_minutes);
                }));
                $processed_count = 0;
                ...
                foreach ($day_minutes as $minute) {
                  if (in_array($minute, $available_minutes)) {
                    ...
                    if ($available_blocks_count) {
                      ...
                      echo '<div class="day-available" style="' . $three_day_class . ';left:' . $left . '%;width:' . $width . '%;"></div>';
                    }
                ...
                if ($available_started_on !== false) {
                  echo '<div class="day-available" style="' . $three_day_class . ';left:' . ($available_started_on / $total_day_minutes_count * 100) . '%;width:' . ($available_blocks_count / $total_day_minutes_count * 100) . '%;"></div>';
                }
              }
              ?>
            </div>
        ...
        public static function get_work_periods_for_date_range($date_from = false, $date_to = false, $args = [])
        {
          ...
          for ($day = clone $date_from_obj; $day <= $date_to_obj; $day->modify('+1 day')) {
            ...
            foreach ($periods[$day->format('N')] as $period) {
              ...
              if ($last_added_period && ($last_added_period['custom_date'] != $period['custom_date'] ||
