  ...
  public static function call_by_route_name($route_name, $return_format = 'html'){
    ...
    if(class_exists($controller_class_name)){
      ...
      if(method_exists($controller_obj, $action)){
        // check permissions
        $logged_in_user_type = OsAuthHelper::get_highest_current_user_type();
        $allow_access = $controller_obj->can_user_type_view_route($action, $logged_in_user_type);
        if($allow_access){
          $controller_obj->route_name = $route_name;
          $controller_obj->$action();
        }else{
          _e('Not Authorized', 'latepoint');
          exit();
        }
      }else{
