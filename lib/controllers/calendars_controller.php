...
if ( ! class_exists( 'OsCalendarsController' ) ) :
  ...
  class OsCalendarsController extends OsController {
    ...
    function __construct(){
      ...
      $this->action_access['public'] = array_merge($this->action_access['public'], ['slot_handle']);
    ...
    function daily_agents(){
      ...
      if($locations){
        ...
        $default_location = OsSettingsHelper::is_on('one_location_at_time') ? false : $locations[0]->id;
        $selected_location_id = isset($this->params['location_id']) ? $this->params['location_id'] : $default_location;
      }else{
    ...
    function daily_agent(){
      ...
      $bookings = OsBookingHelper::get_bookings_for_date($target_date->format('Y-m-d'), ['agent_id' => $selected_agent_id, 'location_id' => $selected_location_id]);
    ...
    function weekly_agent(){
      ...
      if($locations){
        // show all locations option if agent can only be present at one place - it means he does not have overlapping appointments on the calendar
        $default_location = OsSettingsHelper::is_on('one_location_at_time') ? false : $locations[0]->id;
        $selected_location_id = (isset($this->params['location_id']) && is_numeric($this->params['location_id'])) ? $this->params['location_id'] : $default_location;
      }else{
        $selected_location_id = false;
      }
    ...
    function slot_handle(){
      ...
      $query0 = "SELECT start_date,mid_date,end_date FROM ".LATEPOINT_TABLE_BOOKINGS." WHERE agent_id = ".$this->params['agent_id']." AND service_id = '3' AND (start_date = '".$this->params['date']."' OR mid_date = '".$this->params['date']."' OR end_date = '".$this->params['date']."')";
      $fetched_date = $booking->get_query_results($query0);
      $end_date = $fetched_date[0]->end_date;
      if(!($end_date)){
        $end_date = $this->params['date'];
      }
      $query = "SELECT halfday_time FROM " .LATEPOINT_TABLE_BOOKINGS." WHERE end_date = '".$end_date."' AND agent_id = ".$this->params['agent_id']." GROUP BY end_date,halfday_time HAVING COUNT(halfday_time) >=".OsBookingHelper::get_agent_seat_number_by_service(1,$this->params['agent_id']);
      $fetched_date = $booking->get_query_results($query);
